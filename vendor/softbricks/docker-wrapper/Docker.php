...
class Docker
{
     ...
     *
     * @param array|string $args_to_run
     * @return array
     */
    public function executeCommand($args_to_run = [])
    {
        // we needs the "args" argument to be an array
        if (!is_array($args_to_run)) {
            $args_to_run = [ $args_to_run ];
        }
        $outputBuffer = [];
        $to_run_command='docker ' . implode(' ', $args_to_run);
        //print_r($to_run_command);
        exec($to_run_command,$outputBuffer);
        return $outputBuffer;
    }
    ...
    public function bash($name,$command){
        $output = $this->executeCommand([
            $command,
            $name
        ]);
